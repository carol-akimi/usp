...
O fechamento do dia fecha as atividades do dia, mas o programa pode ser modificado no próximo dia.
...
void FechamentoDia(Passageiro *Lugares, int Ocupados);//natalie
/*
...
void *AlocaVet(int Tam, int Tipo);//natalie
/*
    ...
    while(1) {
        ...
        if (!strcmp(Comando, "FV") || InfVoo.QtdAssentos == InfVoo.AssentosOcupados-InfVoo.Canceladas){
            ...
            if (InfVoo.Status == 1){ //o voo está fechado
                ...
                Lugares[AchaReserva(Lugares, InfVoo.AssentosOcupados)].Status = 1;
                InfVoo.Canceladas++;
            }
